...
void makePlots() {
    ...
    for (int irun = minRun; irun <= maxRun; ++ irun) {
        for (int jrbx = 0; jrbx < int(rbxList.size()); ++jrbx) {
            ...
            //sprintf(rbxs,       "rbx==%d && shunt==%d", irbx, shunt);
            if(irbx==14) {
            ...
            rbxHistoMap[irbx]->SetLineColor(cycleColor);
            rbxHistoMap[irbx]->SetFillColor(cycleColor);
            rbxHistoMap[irbx]->GetXaxis()->SetTitle("Max fC");
            rbxHistoMap[irbx]->GetYaxis()->SetTitle("SiPM Channels");
            if (irun == minRun && jrbx == 0) {
            ...
            sprintf(fileName, "RBXs Run %d", irun); 
            C0->SaveAs(runDir + plotDir + fileName + ".pdf");
            C0->SaveAs(runDir + plotDir + fileName + ".png");
            // only increase cycle after 2 runs and 2 RBXs
            // the first run is odd and the first jrbx is even
            printf("%s : run %d : cycle %d : color %d\n", nameMap[irbx].c_str(), irun, cycleCount, cycleColor); 
            ...
            for (int irm = minRM; irm <= maxRM; ++irm) {
                ...
                sprintf(sipms,          "rbx==%d && rm==%d && shunt==%d && run>16", irbx, irm, shunt);
                sprintf(pindiode_0,     "rbx==%d && shunt==%d && pdch==0 && run>16", irbx, shunt);
                sprintf(pindiode_1,     "rbx==%d && shunt==%d && pdch==1 && run>16", irbx, shunt);
    ...
    TLegend *leg1 = new TLegend(0.15,0.55,0.35,0.85); // x1, y1, x2, y2 from 0.0 to 1.0
    leg1->SetHeader("RBX Legend");
    for (std::map<int,TH1F*>::iterator it=rbxHistoMap.begin(); it!=rbxHistoMap.end(); ++it) {
        leg1->AddEntry(it->second,nameMap[it->first].c_str(),"f");
    ...
    TLegend *leg2 = new TLegend(0.65,0.15,0.85,0.45); // x1, y1, x2, y2 from 0.0 to 1.0
    leg2->SetHeader("RBX Legend");
    ...
    for (std::map<int,TGraph*>::iterator it=scatter0Map.begin(); it!=scatter0Map.end(); ++it) {
        ...
        leg2->AddEntry(it->second,nameMap[it->first].c_str(),"p");
        printf("%d --- %s\n", it->first, nameMap[it->first].c_str());
    }
